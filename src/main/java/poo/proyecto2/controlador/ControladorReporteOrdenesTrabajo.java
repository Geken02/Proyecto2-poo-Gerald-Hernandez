...
import poo.proyecto2.vista.reportes.VentanaReporteOrdenesTrabajo; // Referencia a la vista
...
public class ControladorReporteOrdenesTrabajo {

    private SistemaPrincipal modelo;
    private VentanaReporteOrdenesTrabajo vista; // Referencia a la vista

    public ControladorReporteOrdenesTrabajo(SistemaPrincipal modelo, VentanaReporteOrdenesTrabajo vista) {
        this.modelo = modelo;
        this.vista = vista;
    }
    ...
    public void generarPDF() {
        ...
        try {
            ...
            String estadoSeleccionado = vista.getCmbEstado().getSelectedItem().toString();
            String tipoSeleccionado = vista.getCmbTipo().getSelectedItem().toString();
            LocalDate fechaDesde, fechaHasta;
            try {
                fechaDesde = LocalDate.parse(vista.getTxtFechaDesde().getText().trim());
                fechaHasta = LocalDate.parse(vista.getTxtFechaHasta().getText().trim());
                if (fechaHasta.isBefore(fechaDesde)) {
                    vista.mostrarMensaje("La fecha 'Hasta' no puede ser anterior a la fecha 'Desde'.", "Error de Validación", JOptionPane.ERROR_MESSAGE);
                    return;
                }
            ...
            List<OrdenTrabajo> ordenesOriginales = modelo.obtenerTodasLasOrdenes();
